...
**Regra**: sempre executar antes de modificar qualquer código relacionado ao WhatsApp no hermes.
...
## 4. Verificar se hermes usa whatsmeow como dependência

```bash
grep -r "whatsmeow\|whatsapp" /Users/2a/.claude/batalha/hermes-mythos/go.mod 2>/dev/null || \
grep -r "whatsmeow" /Users/2a/.claude/batalha/hermes-mythos/backend/ 2>/dev/null | head -10
```
...
- Breaking changes que podem afetar o hermes
