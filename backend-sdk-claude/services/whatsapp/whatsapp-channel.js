...
const { createRotator: _createConvLogRotator } = require('./log-rotator');
const _convLogRotator = _createConvLogRotator(CONV_LOG_PATH, {
...
// Detecta se o bot foi mencionado ou se a msg é reply a uma msg do bot.
...
// ── Log append-only de conversas (formato blueprint) ──
...
// Normaliza JID → identificador legível (espelha bridge.js upstream).
function _normalizeJid(jid) {
  if (!jid) return '';
  const [num, domain] = String(jid).split('@');
  if (domain === 's.whatsapp.net') return `+${num}`;
  if (domain === 'lid') return `lid:${num}`;
  return jid;
}
